...
|---|---|---|
| Steps 1 & 4 — Code Review | `feature-dev` plugin (`code-reviewer` agent) | `/plugin install feature-dev@claude-plugins-official` |
| Step 2 — Code Simplification | a code-simplifier agent (e.g. `pr-review-toolkit`'s `code-simplifier`) | `/plugin install pr-review-toolkit@claude-plugins-official` |
| Step 3 — Security Review | `/security-review` command | `/plugin install code-review@claude-plugins-official`, or the built-in security review |
| Gate B — Holistic pre-merge review | `/code-review` command | `/plugin install code-review@claude-plugins-official` |

All of the above are available from Anthropic's official marketplace (`anthropics/claude-plugins-official`); substitute equivalents from any other marketplace if you prefer.
